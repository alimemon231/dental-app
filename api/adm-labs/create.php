...
// 1. Capture All Base Payload Fields
...
// Custom Timestamp logic handling if created_at string overrides standard server execution runtime clock
...
// 2. Validate Essential Field Bounds
if (empty($p_id) || empty($case_type_id) || empty($next_visit)) {
    Api::error('Patient selection, Case Type, and Next Visit Procedure are required.');
    exit;
}
...
// 4. Package Compiled Array Block for Clean Table Schema Engine Mapping
$labData = [
    'p_id'            => intval($p_id),
    ...
    // Admin Tracking assignments
    'sent_by'         => $currentUserId,
    'edited_at'       => $baseTimestamp // 'YYYY-MM-DD HH:MM:SS'
];

// 5. DB Execution Sequence Pipeline
try {
    $db->insert('labs', $labData);
    Api::success(1, 'Lab case record synchronized and logged successfully.');
} catch (Exception $e) {
    Api::error('Database Error Execution Failure: ' . $e->getMessage(), 500);
}