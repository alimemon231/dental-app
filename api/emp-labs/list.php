 ...
 * List lab cases for the current user's office with status 'Sent' or 'Received' along with dynamic search matching filters.
 */
...
// Base status constraint check: Must belong to either 'Sent' or 'Received' pipeline scopes
if (!empty($statusFilter)) {
    // If user searches for a specific status explicitly, verify it sits inside allowed list boundaries
    if (in_array($statusFilter, ['Sent', 'Received'])) {
        $whereClauses[] = "l.status = ?";
        $whereParams[]  = $statusFilter;
    } else {
        // If an explicit status is checked that doesn't match base query limits, safely return empty dataset matrix
        Api::success([
            'records'       => [],
            'total_records' => 0,
            'total_pages'   => 0,
    ...
    // Default pool scope: Fetch cases sitting in either 'Sent' or 'Received' matrices
    $whereClauses[] = "(l.status = 'Sent' OR l.status = 'Received')";
}
...
try {
    ...
    Api::error('Lab list pipeline filtering failure: ' . $e->getMessage(), 500);
}