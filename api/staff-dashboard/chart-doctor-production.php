<?php
/**
 * API: Fetch Production Captured By Doctor (Staff Office Scope)
 * Path: /api/staff/chart-doctor-production.php
 * Aggregates current month gross treatment production values grouped by doctor provider, scoped strictly to the user's office.
 */
...
try {
    ...
    // 2. Execute Aggregated Query Grouped by Doctor Provider Join, filtered strictly by office_id
    ...
    foreach ($rows as $row) {
        $labels[] = !empty($row['provider_name']) ? trim($row['provider_name']) : 'Unknown Provider';
    ...
    Api::error('Failed to generate staff doctor production metrics payload: ' . $e->getMessage(), 500);
}