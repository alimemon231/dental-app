 ...
 * Lists unique pre-auth cases alongside their individual itemized treatments matching the 'Approved' status constraints.
 * Filters out individual itemized lines that are not approved, already scheduled, or outside filter boundaries.
 */
...
// Restrict status pooling to 'Approved' base rules while acknowledging drop-down criteria overrides
$statusTargetList = ['Approved'];
if (!empty($statusFilter)) {
    // If user explicitly searches for a non-approved status on the approved panel, return an empty set cleanly
...
try {
    ...
    // 4. Fetch the paginated subset of parent cases containing approved line records
    $caseSql = "SELECT 
                    pac.id AS case_id,
                    pac.patient_id,
                    pac.doctor_id,
    ...
    // 5. Fetch ONLY individual itemized entries matching the target status constraints
    $itemizedSql = "SELECT 
