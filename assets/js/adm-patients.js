/**
 * assets/js/admin-patients.js
 * Admin-level lifecycle CRUD logic for the Patient Management Module.
 * Incorporates global cross-clinic lists, office feeds, and deletion controls.
 */
...
$(document).ready(function () {
    ...
    /* ================================================================
        LOAD TABLE WITH ADM FILTER ATTACHMENTS
    ============================================================ */
    function loadPatients(page) {
        ...
        App.ajax({
            url: '/adm-patient/list.php', // Updated to Admin endpoints for comprehensive global lookup
            method: 'GET',
            loader: false,
            data: {
                page: page,
            ...
            onError: function () {
                $('#patients-tbody').html(
                    '<tr><td colspan="7"><div class="table-empty"><i class="fa-solid fa-circle-exclamation"></i> Failed to load administrative patients list.</div></td></tr>'
                );
            }
        });
    }
    ...
    function renderTable(patients) {
        ...
        $.each(patients, function (i, p) {
            ...
            rows += '<tr>' +
                ...
                '' +
                '<button class="btn btn-ghost btn-sm btn-delete" data-id="' + pId + '" title="Delete Patient Record" style="color:var(--color-danger);"><i class="fa-solid fa-trash-can"></i></button>' +
                '</div>' +
                '</td>' +
                '</tr>';
        });
    ...
    /* ================================================================
        FETCH ALL REGISTERED SYSTEM OFFICES FOR ADMIN DROPDOWNS
    ============================================================ */
    function populateOfficeDropdowns() {
        App.ajax({
            url: '/offices/list.php', // Updated path to fetch all locations globally
            method: 'GET',
            loader: false,
            silent: true,
            ...
            onError: function () {
                console.error("Failed to fetch universal system offices dictionary reference configuration context.");
            }
        });
    }
    ...
    $('#btn-save-patient').on('click', function () {
        ...
        var url = isEditing ? '/adm-patient/update.php' : '/patient/create.php'; // Kept default per instructions
    ...
    $(document).on('click', '.btn-view', function () {
        ...
        App.ajax({
            url: '/patient/get.php?id=' + id, // Kept default per instructions
            loader: false,
    ...
    /* ================================================================
    ADMIN EXCLUSIVE: COMPLETE PATIENT PURGE PIPELINE
================================================================ */
$(document).on('click', '.btn-delete', function () {
    var id = $(this).data('id');
    var name = $(this).data('name') || 'this patient';

    // Reuse the app-wide confirm utility targeting your custom confirm-modal
    App.utils.confirm(
        'Are you sure you want to permanently delete  This will completely wipe all associated pre-authorizations, linked treatment logs, and clinical lab cases across all workspaces.',
        function () {
            App.ajax({
                url: '/adm-patient/delete.php',
                method: 'POST',
                data: { id: id },
                loaderMsg: 'Purging patient architecture metrics...',
                onSuccess: function (d, msg) {
                    App.toast.success('Patient Deleted', msg || 'Record indices successfully dropped.');
                    
                    // Fallback page recalculation if removing the final visible table record row
                    var rowCount = $('#patients-tbody tr').length;
                ...
                onError: function (xhr) {
                    var errorMsg = xhr.responseJSON?.message || 'An error occurred during administrative deletion.';
                    App.toast.error('Deletion Failed', errorMsg);
                }
            });
        }
    );
    ...
    /* ================================================================
        ADMIN EXCLUSIVE DESTRUCTIVE PATIENT LIFE-CYCLE PURGE
    ============================================================ */
    $(document).on('click', '.btn-delete', function () {
        var recordId = $(this).data('id');

        App.modal.confirm({
            message: 'Are you sure you want to permanently delete this patient record? This process is irreversible and will purge associated medical audit history links.',
            onConfirm: function () {
                App.ajax({
                    url: '/adm-patient/delete.php',
                    method: 'POST',
                    data: { id: recordId },
                    loaderMsg: 'Purging profile record indices from infrastructure...',
                    onSuccess: function (res, msg) {
                        App.toast.success('Purged Successfully', msg || 'Patient parameters dropped.');
                        // Check if removing the final card row requires backing up an offset index page
                        var rowCount = $('#patients-tbody tr').length;
                        if (rowCount <= 1 && currentPage > 1) {
                            currentPage--;
                        }
                    ...
                    onError: function (xhr) {
                        var errorContext = xhr.responseJSON?.message || 'Administrative deletion transaction constraint exception.';
                        App.toast.error('Deletion Fault', errorContext);
                    }
                });
            }
        });
