/**
 * Staff Pre-Authorization Monitoring Logic
 * Handles staff pipeline tracking, filtering, and visual status rendering.
 */
...
/**
 * Fetches clinics assigned to the employee to populate the filter dropdown
 */
function loadClinics() {
    App.ajax({
        url: '/auth/user_offices.php',
        onSuccess: function (data, message) {
            let html = '<option value="">Current Clinic</option>';
            data.forEach(clinic => {
...
/**
 * Main data fetcher with multi-dimensional filtering for staff view
 */
function loadStaffData(page = 1) {
    ...
    $('#staff-tbody').html('<tr><td colspan="6" class="text-center"><i class="fa-solid fa-spinner fa-spin"></i> Fetching pipeline data...</td></tr>');

    App.ajax({
        url: '/emp-pre-auth/list-his.php',
        data: filters,
...
function printPipelineTable() {
    const tableHtml = $('#staff-table').clone();
...
function viewLifecycleDetails(id) {
    App.ajax({
        url: '/emp-pre-auth/get.php', // Kept generic for fetching specific record detail context tracking
        method: 'GET',
        data: { id: id },
        loader: true,
        onSuccess: function (data, message) {
            ...
            if (data.procedures_list && data.procedures_list.length) {
                $.each(data.procedures_list, function (idx, item) {
                    listHtml += `<div style="padding: var(--sp-2) 0; border-bottom: 1px dashed #e2e8f0;">
                                    ...
                                    <small style="color:green;display:block;"> Price : $${item.procedure_price}</small>
                                 </div>`;
                });
            } else {
...
function updatePriceMetricsCards(recordsList) {
    ...
    $.each(recordsList, function (index, row) {
        ...
        } else if (status === 'requested' || status === 'sent') {
