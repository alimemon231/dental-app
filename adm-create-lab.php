    ...
    <script>
        $(document).ready(function () {
            ...
            $('#case_type_id').on('change', function () {
                let requirement = $(this).find('option:selected').data('requirement');
                ...
                App.ajax({
                    ...
                    onSuccess: function (res) {
                        ...
                        $.each(dataset.case_types || [], function (i, c) {
                            caseOptions += `<option value="${c.id}" data-requirement="${c.target || 'tooth'}">${App.utils.escHtml(c.name)}</option>`;
                        });
